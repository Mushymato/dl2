...
AMULET_DIR = NEW_SLOT_DIR + '/amulet'
DRAGON_DIR = NEW_SLOT_DIR + '/dragon'
WEAPON_DIR = NEW_SLOT_DIR + '/weapon'
...
def parse_abilities(ability_data):
    ABILITIES_NO_COND = {
        ...
        'Critical Rate': 'cc',
        'Critical Damage': 'cd',
        'Striker\'s Strength': 'sts',
        'Slayer\'s Strength': 'sts',
        'Strength': 'atk',
        'Force Strike': 'fs',
        'Skill Haste': 'sp',
        'Skill Prep': 'prep',
        ...
        'Buff Time': 'bt',
        'Strength Doublebuff': 'def_c_atk',
        'Energy Doublebuff': 'def_c_energy',
        'Last Offense': 'lo',
        'HP &amp; Strength': 'a',
    ...
    ABILITIES_COND = {
        ...
        'Flurry Strength': ('atk', 'hit15'),
        'Whirlwind Strength': ('atk', 'hit15'),
        'Primed Strength': ('skill_link', 'atk'),

        'Stun Res': ('resist', 'stun'),
        'Sleep Res': ('resist', 'sleep'),
        'Burn Res': ('resist', 'burn'),
        'Bog Res': ('resist', 'bog'),
        'Freeze Res': ('resist', 'freeze'),
        'Paralysis Res': ('resist', 'paralysis'),
        'Blindness Res': ('resist', 'blind'),
        'Poison Res': ('resist', 'poison'),
        'Curse Res': ('resist', 'curse'),

        'Burning Punisher': ('k', 'burn'),
        'Paralyzed Punisher': ('k', 'paralysis'),
        'Poison Punisher': ('k', 'poison'),

        "High Midgardsormr's Bane": ('k', 'hms'),
        "High Brunhilda's Bane": ('k', 'hbh'),
        "High Mercury's Bane": ('k', 'hmc'),
        "High Zodiark's Bane": ('k', 'hzd'),
        "High Jupiter's Bane": ('k', 'hjp'),
    }
    SPECIAL = {
        'Strength &amp; Critical Damage I': [('atk', 0.30), ('cd', 0.50)],
        'Strength &amp; Critical Damage II': [('atk', 0.45), ('cd', 0.55)],
        'Dragonyule Blessing I': [('atk', 0.30), ('cc', 0.15)],
        'Dragonyule Blessing II': [('atk', 0.45), ('cc', 0.20)],
        'Strength &amp; Shadow Res II': [('atk', 0.50)],
        'Strength &amp; Wind Res II': [('atk', 0.50)],
        'Strength &amp; Dragon\'s Might I': [('atk', 0.45), ('dc', -2)],
        'Strength &amp; Dragon\'s Might II': [('atk', 0.55), ('dc', -2)]
    }
    SPECIAL_VALUE = {
        'Force Charge III': '25%',
        'Dragon\'s Claws III': 3,
        'Energy Doublebuff I': 1
    }
    # A_TO_AURA = {
    #     's': ('s','passive'),
    #     'a': ('att', 'passive'),
    ...
    #     'sp': ('sp','passive')
    # }
    ABILITY_PATTERN = re.compile(r'(\(([A-Za-z]*)\))?((Full) HP = |HP (\d+)\% = )?\s*(' + '|'.join(ABILITIES_NO_COND.keys())+ r')?(' + '|'.join(ABILITIES_COND.keys())+ r')?\s*(\+\d+\%|[IV]+)')
    parsed = {}
        ...
        if ability_tuple is None:
            ...
            if res:
                ...
                if no_cond is not None:
                    ...
                    continue
                if value[0] == '+':
                    ab_val = int(value[1:-1]) / 100
                else:
                    try:
                        ab_val = SPECIAL_VALUE[v['Name']]
                    except KeyError:
                        continue
...
def get_ability(thingy, abilities, mode='wp', i_range=3, j_range=3, show_desc=True):
    ...
    if mode == 'wp':
        ...
        if len(cond_ab_values) > 0:
            ...
            ability_cond_str = '\n' + ' '*4 + 'def __init__(this, c):\n'
            ability_cond_str += ' '*8 + 'if {}:\n'.format(conditions)
            ability_cond_str += ' '*12 + 'this.a = ' + str(combined_ab)
...
if __name__ == '__main__':
    ...
    with open(AMULET_DIR+'/all.py', 'w') as f:
        ...
        for item in wp_data:
            wp = item['title']
            ab, ab_len = get_ability(wp, ability_data, 'wp', 3, 3, show_desc=False)
            if ab_len == 0:
                continue
